...
class Resnet50():
    ...
    def get_base(self):
        ...
        x = ZeroPadding2D((3, 3), dim_ordering='th')(x)
        x = Convolution2D(64, 7, 7, subsample=(2, 2), name='conv1', dim_ordering='th')(x)
        x = BatchNormalization(axis=bn_axis, name='bn_conv1', dim_ordering='th')(x)
        x = Activation('relu')(x)
        x = MaxPooling2D((3, 3), strides=(2, 2), dim_ordering='th')(x)

        x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), dim_ordering='th')
        x = identity_block(x, 3, [64, 64, 256], stage=2, block='b', dim_ordering='th')
        x = identity_block(x, 3, [64, 64, 256], stage=2, block='c', dim_ordering='th')

        x = conv_block(x, 3, [128, 128, 512], stage=3, block='a', dim_ordering='th')

        for n in ['b','c','d']:
            x = identity_block(x, 3, [128, 128, 512], stage=3, block=n, dim_ordering='th')

        x = conv_block(x, 3, [256, 256, 1024], stage=4, block='a', dim_ordering='th')

        for n in ['b','c','d', 'e', 'f']:
            x = identity_block(x, 3, [256, 256, 1024], stage=4, block=n, dim_ordering='th')

        x = conv_block(x, 3, [512, 512, 2048], stage=5, block='a', dim_ordering='th')
        x = identity_block(x, 3, [512, 512, 2048], stage=5, block='b', dim_ordering='th')
        x = identity_block(x, 3, [512, 512, 2048], stage=5, block='c', dim_ordering='th')
